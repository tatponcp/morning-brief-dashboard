...
import type { Bias, Brief } from "@/lib/types";
    ...
    try {
      ...
      setPublishMsg(
        json.ok
          ? {
              ok: true,
              text: `เผยแพร่ ${json.date} ขึ้นเว็บแล้ว${
      ...
      setPublishMsg({ ok: false, text: "ติดต่อเซิร์ฟเวอร์ไม่ได้" });
    } finally {
      setPublishing(false);
    }
  }
    ...
    <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canPublish && (
              <button
                onClick={publish}
                disabled={publishing}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#34f5a0] to-[#22d3ee] px-3.5 py-2.5 text-[13px] font-semibold text-ink-950 transition hover:brightness-110 disabled:opacity-60"
              >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="hidden items-center gap-1.5 rounded-lg border border-white/10 bg-white/3 px-3 py-1.5 text-[12px] text-slate-400 lg:flex">
          {canPublish ? (
            <>เผยแพร่: กด &ldquo;เผยแพร่ขึ้นเว็บ&rdquo; แล้วลูกค้าเห็นทันที</>
          ) : (
            <>
