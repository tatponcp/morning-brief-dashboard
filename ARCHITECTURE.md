...
## 4.1.5 วิธีใช้งานจริงประจำวัน (ทำงานได้แล้ว ไม่ต้องมี DB)

```
เปิด /studio  →  ใส่รหัส
   ├─ ข้อ 1, 2  : ลากไฟล์ CSV ย้อนหลังเข้ามา ระบบเดาคอลัมน์ให้ → กด "ใช้ข้อมูลนี้"
   ├─ ข้อ 3-6   : วางภาพที่แคปมา → กด "ปักจุดอธิบาย" แล้วคลิกบนภาพ
   └─ ทุกข้อ    : กรอก สรุปสั้น / แปลความ / Action / Insight  (บันทึกร่างอัตโนมัติ)
        ↓
กด "ส่งออก Brief (.json)"  →  ได้ไฟล์ brief-YYYY-MM-DD.json
        ↓
วางทับ  web/src/data/published.json  →  git push
        ↓
Vercel build เอง  →  ลูกค้าเห็นของใหม่ (~1 นาที)
```

**Importer รองรับไฟล์จริง** — คั่นด้วย `,` `;` หรือ tab, ตัวเลขมีลูกน้ำ, วันที่แบบ พ.ศ.
(05/08/2569), หัวคอลัมน์ไทยหรืออังกฤษ, แยกหลายสัญญาจากคอลัมน์ symbol ให้เอง
และถ้าไฟล์เป็นยอดรายวันแทนยอดสะสม ติ๊กออกแล้วระบบบวกสะสมให้
แถวที่อ่านไม่ได้จะถูกข้ามพร้อมบอกเลขบรรทัด ไม่ทำให้ทั้งไฟล์พัง
