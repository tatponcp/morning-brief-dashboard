...
## 4.1.7 หน้าตาใหม่ของ IC Studio

จัดหน้าใหม่เป็น 3 คอลัมน์ แทนการเลื่อนยาวคอลัมน์เดียว

```
[แถบ section]   [พื้นที่แก้ไข (แบ่งแท็บ)]   [พรีวิวสด]
 1 ✓ ครบแล้ว     ┌ ภาพ + จุดอธิบาย ┐
 2 ○ เหลือ 2      │ นำเข้าข้อมูล    │
 3 ✓ ...          └ ข้อความ ────────┘
─────────────────────────────────────────────
 บันทึกอัตโนมัติ    ↶ ↷ ↺   ส่งออก .json   เผยแพร่ขึ้นเว็บ
```

- **แถบซ้ายบอกสถานะรายข้อ** ว่ากรอกครบหรือยัง เหลือช่องไหน (`src/lib/draft-status.ts`)
- **แบ่งแท็บ** ตามชนิดของ section — ไม่ต้องเลื่อนผ่านของที่ไม่เกี่ยว
- **แถบล่างติดหน้าจอ** เห็นสถานะบันทึกและปุ่มเผยแพร่ตลอดเวลา
- **undo / redo** ย้อนได้ 30 ขั้น (Ctrl+Z, Ctrl+Shift+Z)
- **เลือกวันที่ได้** เผยแพร่ของวันไหนก็ได้ ไม่ผูกกับวันที่ในโค้ด
- **ปักจุดบนภาพ** คลิกบนภาพได้เลย ไม่ต้องกดเปิดโหมดก่อน · ลากจุดเพื่อย้าย
- **ปุ่มลัดหัวข้อ Action** ที่ใช้บ่อย กดเพิ่มได้ไม่ต้องพิมพ์
- คีย์ลัด: `Alt+1-6` สลับ section · `Ctrl+Z` ย้อนกลับ · `Enter` ในช่องสรุปสั้น = เพิ่มบูลเล็ต

## 4.1.8 ฝั่งลูกค้า

- **Ctrl+K** เปิดช่องค้นหา section (พิมพ์ไทยหรืออังกฤษก็ได้ เช่น "ทอง", "gold", "breadth")
- **กดเลข 1-6** ข้ามไป section ได้ทันที (ปิดไว้ในหน้า /studio เพื่อไม่ให้ชนกับคีย์ลัดของที่นั่น)
- **ปุ่มก่อนหน้า / ถัดไป** ท้ายทุกหน้า อ่านไล่ทีละข้อได้โดยไม่ต้องกลับไปที่เมนู
